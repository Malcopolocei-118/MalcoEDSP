...
public class YamlDateParser {
    ...
    private static final ImmutableList<String> DATE_TIME_DURATION_UNITS = ImmutableList.of("y", "M", "d", "H", "m");
    ...
    private static String parseToDateStr(String dateStr) {
        ...
        Optional<String> optDateStr = DATE_TIME_DURATION_UNITS
                .stream()
                .filter(offsetStr::contains)
                .map(durationUnit -> {
                    int offsetDuration = Integer.parseInt(remove(offsetStr, durationUnit));
                    Map<String, LocalDateTime> dateUnitMap = getDurationUnitDateTimeMap(offsetDuration);
